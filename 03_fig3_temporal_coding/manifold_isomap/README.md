...
`git_isomap_processing.ipynb` performs Isomap dimensionality reduction on normalized spike-rate data during high-velocity epochs and compares the resulting low-dimensional embedding with the animal’s spatial position. Example input data are provided in `git_folder/SPK_BHV_isomap_input_posBIN.mat`.
